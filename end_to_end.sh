...
bash scripts/run_CoFi.sh $TASK $SUFFIX $EX_CATE $PRUNING_TYPE $SPARSITY $DISTILLATION_PATH $DISTILL_LAYER_LOSS_ALPHA $DISTILL_CE_LOSS_ALPHA $LAYER_DISTILL_VERSION $SPARSITY_EPSILON $BASE_MODEL $SEED

# Run the evaluation script
./test.sh $TASK $SUFFIX $GPUID $BASE_MODEL $EX_CATE $SEED

# Gather the evaluation script results
FTPath=CoFI_runs/$TASK/$EX_CATE/$TASK\_$SUFFIX/$SEED/best
if [ ! -d $FTPath ] 
then
